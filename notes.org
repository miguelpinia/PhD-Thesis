...
** Thread managment
   ...
   ``If =t= is a ~Thread~ object whose thread is currently executing, =t.join()= causes
   the current thread pauses execution until =t='s thread terminates''
...
*** Callables and Futures
    ...
    tasks are called callables (~Callable~ interface). Such interface is similar
    to ~Runnable~, but instead of return void in its =run= method, it returns a
    value. The Callable objects are passed to the method =submit= of the executorService
