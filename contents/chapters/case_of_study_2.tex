...
For the second implementation, we need to make more complex changes. Unlike the single-producer multi-consumer queue used for work-stealing in Chapter~\ref{chapter:4_work-stealing}, we are now dealing with a multi-producer multi-consumer environment. This means we must incorporate a mechanism to ensure that node insertion in the queue is executed correctly.  Algorithms~\ref{alg:basket-queue-linked-list-enq} and~\ref{alg:basket-queue-linked-list-deq} show the necessary changes to convert the Algorithm~\ref{alg:basket-queue} to a queue capable of handling long-run executions where the algorithm may be running.
...
Besides the Segment structure defined, now the shared variables are pointers to nodes of type Segment that represent Head and Tail similar to those used in Michael-Scott's lock-free queue~\cite{DBLP_conf_podc_MichaelS96}. Now, we will prove that the queue defined in Algorithms~\ref{alg:basket-queue-linked-list-enq}
 and~\ref{alg:basket-queue-linked-list-deq} is lock-free linearizable.
\begin{theorem}
...
\begin{proofT}
    ...
    A \Enq operation loops only if the condition in line~\ref{BQ03} is satisfied, the condition in line~\ref{BQ05} is satisfied, the condition in the line~\ref{BQ18} fails, or if the current segment is full and fails in the \CAS in the line~\ref{BQ13}. A \Deq operation loops only if 
\end{proofT}
...
\line{BQ38} \>\>\> {\bf endif} \\
\line{BQ39} \>\>\> $head' = lastHead\rightarrow{}HEAD.\LL()$ \\
\line{BQ40} \>\>\> $tail' = lastHead\rightarrow{}   TAIL.\LL()$\\
\line{BQ41} \>\>\> {\bf if $head == head' == tail' == tail$ then return $\epty$ endif}\\
\line{BQ42} \>\>\> $headTicket = head'$ \\
\line{BQ43} \>\>\> $tailTicket = tail'$\\
\line{BQ44} \>\> {\bf endwhile} \\
\line{BQ4S} \> {\bf endwhile}\\
{\bf end \Deq}
\end{tabbing}
\end{minipage} }
