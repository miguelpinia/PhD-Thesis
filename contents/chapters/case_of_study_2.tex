\chapter{\label{chapter:5_modular-basket-queues}Case Study 2: Modular Baskets Queue}
...
For the second implementation, we need to make more complex changes. Unlike the single-producer multi-consumer queue used for work-stealing in Chapter~\ref{chapter:4_work-stealing}, we are now dealing with a multi-producer multi-consumer environment. This means we must incorporate a mechanism to ensure that node insertion in the queue is executed correctly.  Algorithms~\ref{alg:basket-queue-linked-list-enq} and~\ref{alg:basket-queue-linked-list-deq} show the necessary changes to convert the Algorithm~\ref{chapter-5:alg:basket-queue} to a queue capable of handling long-run executions. We use the operator \(\rightarrow\) to denote the access to elements in a structure.
...
Besides the new Segment structure, now the shared variables are pointers to nodes of type Segment that represent Head and Tail similar to those used in Michael-Scott's lock-free queue~\cite{DBLP_conf_podc_MichaelS96}. We will prove that the queue defined in Algorithms~\ref{alg:basket-queue-linked-list-enq} and~\ref{alg:basket-queue-linked-list-deq} is lock-free and linearizable.
...
\begin{proofT}
  ...
  We begin analyzing the \Enq operation. We observe that a \Enq operation enters a loop when:

  \begin{itemize}
      \item \(lastTail\) is not equals to \(Tail\) in line~\ref{BQ03}. 
      \item \(lastNext\) is not null in line~\ref{BQ05}. 
      \item The current segment is full and fails the \CAS at line~\ref{BQ13}.
      \item The thread is unable to insert the value in the respective basket at line~\ref{BQ18}.
  \end{itemize}

  Now, we will demonstrate that the \Enq operation is lock-free by proving that a process only loops beyond a finite number of times if another process completes an \Enq on the queue.

  \begin{itemize}
    \item At line~\ref{BQ03} the condition is satisfied only if the pointer to the Tail has changed; this means that if another process has updated the reference to the segment, then that process must have successfully completed an enqueue operation.
    \item At line~\ref{BQ05} the condition is satisfied only if, while reading the Tail pointer, another process appends a new segment (being succeeded in inserting a new element), and the process still does not update the reference to the Tail pointer. In such a case, we will try to help update the reference to the Tail segment and loop again.
    \item At line~\ref{BQ13} the condition fails only if the \CAS cannot append a new segment; this means that another process appends its segment and succeeds in inserting a new element into the queue.
    \item At line~\ref{BQ18} the condition fails only if the operation cannot insert an element into the basket.
  \end{itemize}

  Now, we observe that a \Deq operation enters a loop under when:

  \begin{itemize}
      \item \(lastHead\) is not equals to\(Head\) in line~\ref{BQD04}.
      \item The inner cycle encompassing lines~\ref{BQD11} to~\ref{BQD22} has been completed, and it does not return any output.
  \end{itemize}
  
  We will show that the \Deq operation in Algorithm~\ref{alg:basket-queue-linked-list-deq} is lock-free by performing a similar analysis to ours for the \Enq operation. This will involve proving that a process only loops beyond a finite number of times if another process successfully completes a \Deq operation on the queue.

  \begin{itemize}
    \item At line~\ref{BQD04}, the condition is satisfied if the pointer to \(lastHead\) is distinct from the current pointer to \(Head\); in such case, we must loop again.
