...
\begin{proofT}
  ...
  \begin{itemize}
    ...
    \item vWit: A \Deq operation returning \epty even though the queue is never logically empty during the execution of the \Deq operation. An item is logically in the queue if it is in a basket \(items[i]\) and \(i < TAIL\). When a \Deq operation returns \epty, there is a point in the time where no basket in \(items[0, 1, \ldots, TAIL - 1]\) contains an item, and hence the queue is logically empty (it might, however, be the case that \(items[TAIL]\) does contain an item at that moment). Hence, every execution is free of VWit.
  \end{itemize}
