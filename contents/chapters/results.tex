...
\begin{itemize}
    \item \textbf{LL/IC performance comparison}: In this experiment, taking as reference the \FAI instruction performance, we observe that \CAS-based LL/IC object had the best performance in almost all cores, followed by the mixed version of LL/IC and finally the RW-based with 64 of padding for each entry.
    \item \textbf{Modular queue variants comparison}:
    \item \textbf{Queue comparison}:
\end{itemize}