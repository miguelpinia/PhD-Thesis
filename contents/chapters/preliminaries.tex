...
In addition to the mathematical computation model, it is important to
know some hardware foundations to explain why it is necessary use some
tools as (for example, \emph{fences}) to perform concurrent computing
correctly. In this section, we will discuss about the following
topics:
...
Each processor uses the first level of cache to get the data and
instructions to execute them; usually, the access to this level of
cache is high-speed concerning access to other levels. The second
level is often more extensive and stores data and instructions close
to being executed. In the third level, this cache is shared by many
