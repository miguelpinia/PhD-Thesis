...
In addition to the mathematical computation model, understanding the hardware foundations behind concurrent computing is important. This understanding helps explain why certain tools, such as ``\emph{fences}'' are necessary to ensure correct concurrent computing. In this section, we will discuss the high-level working of concurrent programs on computer hardware. Our focus will be on memory interactions at different levels. We will cover the following topics in detail: Cache memory, memory bus, main memory, consistency memory model, cache coherence
...
In this case, we might expect that the \texttt{print(y)} always print the number 30, nevertheless, the compiler or the CPU could change the order of the instructions for thread 2, giving, as a result, an execution where the value for \texttt{y} is undefined, and the instructions could be interleaved as follows:
